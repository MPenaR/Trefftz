from cupy import dot, exp, sinc, pi
from trefftz.numpy_types import float_array, complex_array
from trefftz.dg.exact import PlaneWave
import cupy as cp
from cupy.typing import NDArray

def I_uv(segment, D_u: NDArray[cp.float64], n_u: float,  D_v: NDArray[cp.float64], n_v: float,  k: float) -> NDArray[cp.complex128]:
    r'''Computes the integral:
    .. math ::
        \int_E u \overline{v}\,\mathrm{d}\ell

    where $u$ and $v$ are plane waves and $E$ is a segment.'''
    l = cp.asarray(segment["l"])
    M = cp.asarray(segment["M"])
    T = cp.asarray(segment["T"])
