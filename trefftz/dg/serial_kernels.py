...
def I_udv_segment(segment, d_u: float_array, d_v: float_array, k: float) -> complex:
    ...
    T = segment["T"]
    N = segment["N"]
    return -1j*k*l*exp(1j*k*dot((d_u - d_v), M))*dot(d_v, N)*sinc(k*l/(2*pi)*dot(d_u - d_v, T))
...
def I_udv_arc(arc, d_u: float_array, d_v: float_array, k: float) -> complex:
    ...
    primitive = lambda theta: -k*R*(-jv(1, k*R*D_uv)*cos(phi_uv - phi_v)* theta + sum(1j**p/p*(jv(p-1,k*R*D_uv)*sin(p*(theta-phi_uv)+(phi_uv-phi_v))-
...
def I_dudv_segment(segment, d_u: float_array, d_v: float_array, k: float) -> complex:
    ...
    T = segment["T"]
    N = segment["N"]
    return k**2*l*exp(1j*k*dot((d_u - d_v), M))*dot(d_u, N)*dot(d_v, N)*sinc(k*l/(2*pi)*dot(d_u - d_v, T))
