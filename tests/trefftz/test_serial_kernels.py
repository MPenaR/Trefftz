...
def test_Iuv(d_m, d_n):
    I_exact = exact.I_uv(edge=EDGE_1, d_u=d_n, d_v=d_m, k=k)
    I_num = numerical.I_uv(edge=EDGE_1, d_u=d_n, d_v=d_m, k=k)
...
def test_Iuv_arc(d_m, d_n):
    ...
    I_exact = exact.I_uv_arc(edge=ARC_1, d_u=d_n, d_v=d_m, k=k)
    I_num = numerical.I_uv_arc(edge=ARC_1, d_u=d_n, d_v=d_m, k=k)
    assert_allclose(I_num, I_exact, rtol=TOL, atol=TOL)
...
def test_Iduv_arc(d_m, d_n):
    I_exact = exact.I_duv_arc(edge=ARC_1, d_u=d_n, d_v=d_m, k=k)
    I_num = numerical.I_duv_arc(edge=ARC_1, d_u=d_n, d_v=d_m, k=k)
    assert_allclose(I_num, I_exact, rtol=TOL, atol=TOL)