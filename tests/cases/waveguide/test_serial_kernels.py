...
def test_IuNv(edges, d_m, d_n):
    edge_u, edge_v = edges
    I_exact = exact.I_uNv(edge_u=edge_u, edge_v=edge_v, d_u=d_n, d_v=d_m, k=k, H=H, NtD_modes= NTD_MODES)
    I_num = numerical.I_uNv(edge_u=edge_u, edge_v=edge_v, d_u=d_n, d_v=d_m, k=k, H=H, NtD_modes= NTD_MODES)
